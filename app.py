...
# --- TAB 6: MASTER REPORT (Comprehensive with Tooltips) ---
with tab6:
    st.markdown("""
        <div style="background-color: #111; padding: 20px; border-radius: 10px; border-left: 5px solid #FFCC00; margin-bottom: 25px;">
    ...
    # 3. SECOND ROW: THE MARKETING MANEUVER BOARD
    ...
    m1.metric("Marketing Guests", f"{total_mkt_guests:,.0f}", 
              help="Guests generated by the 'Digital Awareness Pool' (Adstock) and 'OOH Pressure' (Billboards).")
    m2.metric("Mkt. Revenue Impact", f"${mkt_revenue_impact:,.2f}", 
              help="Net GGR specifically attributed to your marketing maneuvers.")
    m3.metric("Market Capture Rate", f"{capture_rate:.1f}%", 
              help="The percentage of total property revenue that was directly driven by marketing efforts.")
