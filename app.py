...
with tab5:
    ...
    if st.button("🗑️ Reset Forensic Session", key="reset_chat_t5"):
        st.session_state.messages = []
        st.rerun()
# --- MASTER REPORT (Tab 6 Snippet) ---
with tab6:
    st.write("## 📋 Master Forensic Report")
    ...
    with col2:
        # THE NEW METRIC
        st.metric("OOH Inertia (Daily)", f"{ooh_daily:.0f} Guests", 
                  help="The constant daily guest lift generated by Static and Digital Billboards.")
    with col3:
        st.metric("Model Predictability", metrics['predictability'])
    ...
    st.info(f"""
        **Forensic Insight:** Your current marketing mix provides a baseline 'Inertia' of **{ooh_daily * 7:,.0f} guests per week**. 
        This represents the physical brand pressure of the 'Your Turn to Hit' campaign in the Ottawa market, independent of digital performance.
    """)
