...
# --- TAB 4: ADVANCED CALIBRATION (Restoring OOH & Gravity Logic) ---
with tab4:
    st.markdown("""
        <div style="background-color: #111; padding: 20px; border-radius: 10px; border-left: 5px solid #FFCC00; margin-bottom: 25px;">
            <h2 style="color: #FFCC00; margin: 0;">⚙️ Forensic Engine Calibration</h2>
            <p style="color: #888; margin: 0;">Fine-tune OOH Pressure, Digital Weights, and Entertainment Gravity.</p>
        </div>
    """, unsafe_allow_html=True)

    # Safety wrapper for values
    def safe_v(key, default, min_v, max_v):
        val = float(st.session_state.coeffs.get(key, default))
        return max(min(val, max_v), min_v)

    with st.form("advanced_calib_v10"):
        # 1. THE OOH ROTATION STACK (The "Modular" Logic)
        st.write("### 🏢 Out-of-Home (OOH) Pressure Stack")
        st.caption("Adjust the baseline guest inertia generated by rotating OOH creative.")
        o1, o2, o3 = st.columns(3)
        with o1:
            n_ooh_brand = st.number_input("Brand/Logo Pressure", 0, 500, int(safe_v('OOH_Brand', 150, 0, 500)))
        with o2:
            n_ooh_jackpot = st.number_input("Jackpot/Gaming Rotation", 0, 500, int(safe_v('OOH_Jackpot', 75, 0, 500)))
        with o3:
            n_ooh_concert = st.number_input("Concert/Live Rotation", 0, 500, int(safe_v('OOH_Concert', 100, 0, 500)))

        st.divider()

        # 2. DIGITAL & GRAVITY
        st.write("### 📣 Awareness & Gravity")
        d1, d2, d3 = st.columns(3)
        with d1:
            n_clicks = st.slider("Ad Click Weight", 0.0, 1.0, safe_v('Clicks', 0.05, 0.0, 1.0))
        with d2:
            n_imp = st.slider("Social Weight", 0.0000, 0.0050, safe_v('Impressions', 0.0002, 0.0, 0.005), format="%.4f")
        with d3:
            n_gravity = st.slider("LIVE Gravity (%)", 0.0, 100.0, safe_v('event_gravity', 25.0, 0.0, 100.0))

        st.divider()

        # 3. THE FINANCIAL FLOOR
        st.write("### 💰 Financial Property DNA")
        f1, f2, f3 = st.columns(3)
        with f1:
            n_spend = st.number_input("Avg Spend / Head ($)", 0.0, 2000.0, safe_v('Avg_Coin_In', 112.50, 0.0, 2000.0))
        with f2:
            n_theo = st.number_input("Property Theo ($)", 0.0, 5000.0, safe_v('Property_Theo', 450.0, 0.0, 5000.0))
        with f3:
            n_hold = st.slider("House Hold %", 0.0, 50.0, safe_v('Hold_Pct', 10.0, 0.0, 50.0))

        # THE SUBMIT BUTTON
        submit_all = st.form_submit_button("🚀 Commit Weights & Re-Sync Engine", use_container_width=True)

        if submit_all:
            # Re-calculating the TOTAL OOH DAILY to feed into Tab 1 and Tab 6
            total_ooh_inertia = n_ooh_brand + n_ooh_jackpot + n_ooh_concert
            
            st.session_state.coeffs = {
                'OOH_Brand': n_ooh_brand,
                'OOH_Jackpot': n_ooh_jackpot,
                'OOH_Concert': n_ooh_concert,
                'OOH_Daily': total_ooh_inertia, # This is the "Inertia" value for the engine
                'Clicks': n_clicks,
                'Impressions': n_imp,
            ...
            st.success(f"✅ Weights Committed. Total OOH Inertia set to {total_ooh_inertia} guests/day.")
            st.balloons()
