...
# 3. PERMANENT INITIALIZATION & SENTIMENT LEDGER

# =================================================================

def initialize_sentiment_ledger():

    file_path = 'sentiment_history_2.csv'

    # Required columns from Social Media Sentiment Scoring.docx

    columns = ["Message_ID", "Timestamp", "Raw_Text", "Asset", 

               "Sentiment_Score", "Sentiment_Category", "Intensity_Level"]
...
initialize_sentiment_ledger()



# =================================================================

# 4. GLOBAL PAGE CONFIG & EXECUTIVE THEME
...
def get_forensic_metrics(df_input, coeffs):
    ...
    # Fix: Define 'is_closed' BEFORE the prediction function runs

    df['is_closed'] = df.apply(lambda x: 1 if (x['entry_date'] < today and x.get('actual_traffic', 0) == 0) else 0, axis=1)

    

    # Fix: Force attendance to float so 1,900 adds ~1,615 guests at 85%
    ...
    def predict_guests(row):
        ...
        # Now 'is_closed' is guaranteed to exist in the row
...
def archive_sentiment_entry(raw_text, asset_name, nlp_score):
    ...
    Follows forensic requirements from 'Social Media Sentiment Scoring.docx'.

    """

    # 1. Determine Category and Icon
    ...
    # 4. Push to Supabase Cloud[cite: 1]
...
def draw_sentiment_gauge(score):
    ...
    🔴 Negative < -0.3 | 🟡 Neutral -0.3 to 0.3 | 🟢 Positive > 0.3
    ...
    # Initialize a polar plot for the gauge effect

    fig, ax = plt.subplots(figsize=(5, 2.5), subplot_kw={'projection': 'polar'})

    

    # Define Color Zones: Red, Yellow, Green based on doc thresholds

    colors = ['#ff4b4b', '#fbc02d', '#00c853']

    

    # Background segments (mapping the -1 to +1 scale to pi to 0 radians)

    # Negative Zone[cite: 1]
    ...
    # Positive Zone[cite: 1]

    ax.barh(1, 0.35*np.pi, left=0, color=colors[2], height=0.5)          

    

    # Calculate Needle Position: (1 - normalized score) * pi

    pos = (1 - ((score + 1) / 2)) * np.pi

    ax.annotate('', xy=(pos, 1.1), xytext=(0, 0),

                arrowprops=dict(facecolor='#1A1A1B', width=2, headwidth=8))

    

    # Executive Formatting: Remove clutter for a clean, intuitive look[cite: 1]
...
# --- SECTION 8: EXECUTIVE NAVIGATION ---
...
# --- 9.5 BRAND SENTIMENT PULSE (Step 5 - Move the Needle) ---
        ...
        # Pull the latest trend from the historical ledger

        score_val = 0.0

        sentiment_file = 'sentiment_history_2.csv'

        

        if os.path.exists(sentiment_file):

            df_s = pd.read_csv(sentiment_file)

            if not df_s.empty:

                # Average the last 7 entries for 'Overall Property' to show current trend[cite: 1]

                prop_data = df_s[df_s['Asset'] == 'Overall Property']

                score_val = prop_data['Sentiment_Score'].tail(7).mean() if not prop_data.empty else 0.0
        ...
        with s_col1:
            ...
            # Render the Gauge Visualization[cite: 1]

            gauge_fig = draw_sentiment_gauge(score_val)

            st.pyplot(gauge_fig, use_container_width=True)

            st.caption(f"**Forensic Property Temperature:** {score_val:+.2f}[cite: 1]")



        with s_col2:

            # Executive "Move the Needle" Interpretation[cite: 1]
            ...
            if score_val > 0.3:
                ...
                st.success("**Strategic State: Marketing Velocity**\n\n"
                           ...
                           "Your current 'Event Gravity' and 'Promo Lift' will perform at peak efficiency.[cite: 1]")
                ...
                st.error("**Strategic State: Marketing Tax**\n\n"
                         ...
                         "You are paying a 'tax' on your ad spend to overcome guest hesitation.[cite: 1]")

            else:

                st.warning("**Strategic State: Neutral Friction**\n\n"

                           "The needle is in the neutral zone. "

                           "Brand health is stable, but not currently providing a competitive lift to floor traffic.[cite: 1]")
...
# =================================================================
    ...
    # --- 14.1 SENTIMENT STRATEGIC INPUT (Step 3) ---
    ...
    with st.expander("📝 Manual Social Media Sentiment Entry", expanded=False):
        ...
        with st.form("sentiment_input_form", clear_on_submit=True):
            ...
            f_text = st.text_area("Social Media Message / Comment Text", 
            ...
            submit_sentiment = st.form_submit_button("🛡️ Analyze & Archive to Ledger")
            ...
            if submit_sentiment and f_text:
                ...
                if category != "Error":
                    ...
                    st.info(f"This record is now saved in 'sentiment_history_2.csv' and is accessible to the AI Analyst.")
    ...
    # 2. Fetch Historical Sentiment Data[cite: 1]

    sentiment_file = 'sentiment_history_2.csv'

    if os.path.exists(sentiment_file):

        df_sent_hist = pd.read_csv(sentiment_file)

        sentiment_context = df_sent_hist.tail(15).to_csv(index=False)

    else:

        sentiment_context = "No historical sentiment data available yet."



    # 3. Fetch Monthly ROI Data
    ...
    - Promo: {c.get('Promo')} | OOH: {c.get('OOH_Weight')} | PR: {c.get('PR_Weight')}



    --- HISTORICAL BRAND SENTIMENT DATA (From sentiment_history_2.csv) ---
    ...
    if prompt:
        ...
        try:
            ...
            with st.status("🕵️ Correlating Brand Value with Floor Cash...", expanded=True) as status:
                ...
                Identify if marketing spend is driving quality trips or just volume using these datasets:
